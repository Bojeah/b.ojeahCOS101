...
println!("How many staff member do you want to validate staff level for? ");
...
for i in 1..=staff_num{
    ...
    if staff_occu == "OFFICE ADMINISTRATOR"{
      ...
      rank.push(staff_rank);
        ...
        let mut input4 = String::new();
        println!("\nStaff Occupation (Placement/Classroom Teacher/Senior Teacher/Leading Teacher/Deput Principal/Principal): ");
        io::stdin().read_line(&mut input4).expect("Failed to read input");
        let staff_rank = input4.trim().to_uppercase();
